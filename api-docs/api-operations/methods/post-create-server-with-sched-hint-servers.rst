...
within a specific zone. Use either the ``public-zone-ip:near`` or the ``public-zone-ip:far`` 
parameter and be sure to use the server`s ``ID`` and not the server's ``zone`` when making 
the request.
...
The ``public-zone-ip:near`` and ``public-zone-ip:far`` parameters allow you specify up to 
three server IP addresses, but we recommend you use just one IP address to reduce chance of 
failure.
